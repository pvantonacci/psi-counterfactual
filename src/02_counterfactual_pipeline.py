...
# ── Charlson ICD-10 prefix map  (Quan et al. 2005) ───────────────────────────
# Dict: category_name → (weight, [ICD-10-CM prefix list])
# Used in Stage 2a to add comorbidity features to the propensity-score matrix.
_CHARLSON_PREFIXES: dict[str, tuple[int, list[str]]] = {
    "MI":          (1,  ["I21","I22"]),
    ...
    "CEREB":       (1,  ["G45","G46","H34.0","I60","I61","I62","I63","I64","I65",
                          "I66","I67","I68","I69"]),
...
def _charlson_from_codes(codes: list[str]) -> tuple[int, dict[str, int]]:
    """
    Returns (total_charlson_score, {category: 0/1}) for a list of ICD-10-CM codes.
    Applies standard hierarchy: DM-complicated suppresses DM-uncomplicated;
    mod/severe liver suppresses mild liver; metastatic suppresses non-metastatic cancer.
    """
    code_clean = [c.upper().replace(".", "") for c in codes if c and str(c) != "nan"]
    flags: dict[str, int] = {}
    for cat, (_, prefixes) in _CHARLSON_PREFIXES.items():
        clean_prefixes = [p.upper().replace(".", "") for p in prefixes]
        flags[cat] = int(any(c.startswith(p) for c in code_clean for p in clean_prefixes))
    # Hierarchy
    if flags.get("DM_COMP"):  flags["DM_UNCOMP"]  = 0
    if flags.get("MOD_LIVER"): flags["MILD_LIVER"] = 0
    if flags.get("METASTATIC"): flags["CANCER"]    = 0
    score = sum(_CHARLSON_PREFIXES[cat][0] * v for cat, v in flags.items())
    return score, flags
...
def _run_pipeline():
    ...
    def build_features_at_tstar(
        ...
        # ── Charlson comorbidity from pre-existing diagnoses ──────────────────────
        # Pre-existing: DX_CHRONIC='YES' or DX_DATE < t0 or DX_STATUS POA.
        # Only pre-admission comorbidity is included — no post-event leakage.
        if enc_id in dx_by_enc:
            enc_dx_all = dx_by_enc[enc_id]
            pre_mask = pd.Series(False, index=enc_dx_all.index)
            if "DX_CHRONIC" in enc_dx_all.columns:
                pre_mask |= enc_dx_all["DX_CHRONIC"].fillna("").astype(str).str.upper() == "YES"
            if "DX_DATE" in enc_dx_all.columns:
                _pre_dates = pd.to_datetime(enc_dx_all["DX_DATE"], errors="coerce")
                pre_mask  |= _pre_dates.notna() & (_pre_dates < t0)
            if "DX_STATUS" in enc_dx_all.columns:
                pre_mask |= enc_dx_all["DX_STATUS"].fillna("").astype(str).str.upper().str.contains(
                    "POA|BEFORE", regex=True, na=False)
            pre_codes = enc_dx_all.loc[pre_mask, "DX_CODE"].dropna().astype(str).tolist() \
                        if "DX_CODE" in enc_dx_all.columns else []
            cs_score, cs_flags = _charlson_from_codes(pre_codes)
            feats["charlson_score"] = cs_score
            feats["n_preexisting_dx"] = int(pre_mask.sum())
            for cat, flag in cs_flags.items():
                feats[f"cs_{cat}"] = flag
