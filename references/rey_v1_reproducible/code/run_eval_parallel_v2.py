...
PROMPT_TEXT = {
    ...
    "C8": ("Summarize the lab results below. Identify which values are abnormal, flagging each "
...
JUDGE_SYSTEM = """You are an evaluator scoring a medical AI model's response against a rubric criterion.
Base your judgment ONLY on the source material and the model response provided.
Do not apply outside clinical knowledge — score only what is stated in the text, not what would be clinically ideal or expected.
